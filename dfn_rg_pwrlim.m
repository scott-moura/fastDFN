...
for k = 1:(NT-1)
    ...
    % Power Limit Estimation via Reference Governor
    beta_max(k) = bisection_dfn_pwrlim(p,x(:,k),z(:,k),I(k), 67);  % 1C corresponds to 67Ah/m2 for LiCoO2
    beta_min(k) = bisection_dfn_pwrlim(p,x(:,k),z(:,k),I(k), -67);  % 1C corresponds to 67Ah/m2 for LiCoO2
    
%     % Reference Governor
%     if(Ir(k) ~= 0)
%         beta(k) = bisection_dfn_pwrlim(p,x(:,k),z(:,k),I(k),Ir(k)); %changed to federico version NS July20
%     else
%         beta(k) = 1;
%     end
%     I(k+1) = beta(k) * Ir(k);
